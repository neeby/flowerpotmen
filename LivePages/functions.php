...
function addHouse($Address,$Suburb,$PostCode,$BedRooms,$BathRooms,$CarPorts,$Description,$Image,$WeeklyRent,$Owner){
    ...
    if($number[0]=="0"){
    	...
    	$sql = "INSERT INTO `Properties` (`Code`,`Address`, `Suburb`, `Postcode`, `BedRooms`, `BathRooms`, `CarPorts`, `Description`, `Image`, `WeeklyRent`,`Verified`,`Owner`,`Staff`) VALUES (";
    $sql .= "'{$Code}', ";
    $sql .= "'{$Address}', ";
	$sql .= "'{$Suburb}', ";
	$sql .= "'{$PostCode}', ";
...
*/
    ...
    function writeMenu(){
        ...
        $OwnerMenuList = array(hyper("Home"), hyper("Properties"), hyper("Contact"), hyper("Logout"));
        $AdminMenuList = array(hyper("Home"), hyper("Properties"), hyper("Users"), hyper("Staff"), hyper("Contact"), hyper("Logout"));
        $StaffMenuList = array(hyper("Home"), hyper("Properties"), hyper("Users"), hyper("Staff"), hyper("Contact"), hyper("Logout"));
	...
	$query = "SELECT * FROM Properties WHERE Verified =1";
		...
		if (!empty($_GET['searchParam'])){
			if ($_GET['searchParam'] == 'myProperties'){
				$query = "SELECT * FROM Properties WHERE Owner = '{$_SESSION['POST_User']}' AND Verified = 1";
			} else {
				$query = "SELECT * FROM Properties WHERE Verified = 1   ";	
			}
...
//--
                ...
                if ($ImageAddress == ""){
                    $TableToPrint = $TableToPrint . "</td><td rowspan='5' style='vertical-align: middle;'><b>Photo missing.</b><br><img src='images/pingu.jpg' class='SolidBorder'>"; 
                } else {
                    $TableToPrint = $TableToPrint . "</td><td rowspan='5'><b>Photo:</b><br><img src=" . $SearchResults[$i][8] . " class='SolidBorder'>";
                }
    ...
    function echoMap($AddressOriginal){
        ...
        */
        ...
        if ($status=="OK") {
            ...
            $LatLng = "$Lat,$Lon";
            echo <<<EOL
<div id="map" style="width:500px;height:500px;margin:auto;margin-right:100%;"></div>
